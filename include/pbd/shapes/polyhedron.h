...
namespace pbd::shapes {
	...
	struct polyhedron {
		...
		struct properties {
			...
			[[nodiscard]] body_properties get_body_properties(double density) const {
				mat33d c = density * covariance_matrix;
				return body_properties::create(mat33d::identity() * c.trace() - c, volume * density);
			}
		...
		/// \ref body_properties.
		[[nodiscard]] body_properties bake(double density) {
