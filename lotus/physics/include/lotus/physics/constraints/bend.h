	...
	struct bend {
		...
		public:
			...
			[[nodiscard]] inline static constraint_state from_rest_pose(
				...
				scalar inv_n1_norm = 1.0 / n1.norm();
				scalar inv_n2_norm = 1.0 / n2.norm();
				n1 *= inv_n1_norm;
				n2 *= inv_n2_norm;

				scalar cosv = vec::dot(n1, n2);
				vec3 sin_vec = vec::cross(n1, n2);
				scalar sinv = vec::dot(sin_vec, d1n);
				scalar theta = std::atan2(sinv, cosv);
		...
		void project(
			...
			vec3 d2 = x3 - x1;
			vec3 d3 = x4 - x1;
			scalar inv_d1_norm = 1.0 / d1.norm();
			vec3 d1n = d1 * inv_d1_norm;
			...
			mat33s i_minus_d1nd1nt_over_d1norm = inv_d1_norm * (mat33s::identity() - d1n * d1n.transposed());
			mat33s i_minus_n1n1t_over_n1norm = inv_n1_norm * (mat33s::identity() - n1 * n1.transposed());
			mat33s i_minus_n2n2t_over_n2norm = inv_n2_norm * (mat33s::identity() - n2 * n2.transposed());
			mat33s n2_cross_i_minus_n1n1t_over_n1norm = vec::cross_product_matrix(n2) * i_minus_n1n1t_over_n1norm;
			mat33s n1_cross_i_minus_n2n2t_over_n2norm = vec::cross_product_matrix(n1) * i_minus_n2n2t_over_n2norm;
			...
			scalar c_coefficient = state.sqrt_sum_inverse_areas * state.edge_length / std::sqrt(8.0);

			scalar theta_diff = clamp_angle(theta - state.rest_angle);
			scalar c = c_coefficient * theta_diff;

			scalar alpha_hat = properties.inverse_stiffness * inv_dt2;
			scalar delta_lambda = -(c + alpha_hat * lambda) / (
