...
namespace g_buffer {
	...
	contents decode(
		...
		float2 uv, float4x4 inverse_jittered_projection_view, float2 depth_linearize
	) {
		...
		result.fragment.position_ws = mul(inverse_jittered_projection_view, position_cs).xyz * result.linear_depth;
