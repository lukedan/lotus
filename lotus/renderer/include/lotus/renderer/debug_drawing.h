	...
	public:
		...
		[[nodiscard]] inline static debug_renderer create(assets::manager &man) {
			...
			result._vertex_shader_textured = man.compile_shader_in_filesystem(
				man.shader_library_path / "debug_textured.hlsl", gpu::shader_stage::vertex_shader, u8"main_vs"
			);
			result._pixel_shader_textured = man.compile_shader_in_filesystem(
				man.shader_library_path / "debug_textured.hlsl", gpu::shader_stage::pixel_shader, u8"main_ps"
			);
