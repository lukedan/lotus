...
namespace lotus {
	namespace dds {
		...
		public:
			...
			[[nodiscard]] static gpu::format four_cc_to_format(u32);
			...
			[[nodiscard]] const header &get_header() const {
				return *_data_at_offset<header>(sizeof(u32));
			}
			...
			[[nodiscard]] const header_dx10 *get_dx10_header() const {
				return
					_has_dx10_header ?
					_data_at_offset<header_dx10>(sizeof(u32) + sizeof(header)) :
					nullptr;
			}
			...
			[[nodiscard]] u32 get_array_size() const {
				return _array_size;
			}
			/// Returns the number of mips.
			[[nodiscard]] u32 get_num_mips() const {
				return _num_mips;
			}
		...
		private:
			...
			usize _size = 0; ///< The size of the binary.

			bool _has_dx10_header = false; ///< Whether a DX10 header is available.

			bool _is_cubemap = false; ///< Whether this is a cubemap texture.
			u32 _width      = 0; ///< The width of the texture.
			u32 _height     = 0; ///< The height of the texture.
			u32 _depth      = 0; ///< The depth of the texture.
			u32 _array_size = 0; ///< Number of array slices in the texture.
			u32 _num_mips   = 0; ///< Number of mips.
			...
			///           be returned.
			template <typename T = std::byte> const T *_data_at_offset(usize off) const {
