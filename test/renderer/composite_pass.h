...
public:
	...
	composite_pass(gfx::device &dev) :
		...
		_pass_resources(dev.create_pass_resources(
			{
				gfx::render_target_pass_options::create(gfx::format::r8g8b8a8_unorm, gfx::pass_load_operation::preserve, gfx::pass_store_operation::preserve),
			},
		...
		_gbuffer_descriptors_layout(dev.create_descriptor_set_layout(
			...
			gfx::shader_stage_mask::compute_shader
		)),
		_pipeline_resources(dev.create_pipeline_resources({ &_gbuffer_descriptors_layout }))
	{
		auto vs_binary = load_binary_file("composite.vs.o");
		auto ps_binary = load_binary_file("composite.ps.o");
		...
		_pipeline_state = dev.create_graphics_pipeline_state(
			...
			gfx::blend_options::create_for_render_targets(gfx::render_target_blend_options::disabled()),
	...
	void record_commands(
		...
		list.bind_graphics_descriptor_sets(0, { &input_rsrc.gbuffer_descriptor_set });
	...
	[[nodiscard]] output_resources create_output_resources(gfx::device &dev, gfx::image2d &img, cvec2s size) const {
		...
		result.image_view = dev.create_image2d_view_from(img, gfx::format::r8g8b8a8_unorm, gfx::mip_levels::only_highest());
		result.frame_buffer = dev.create_frame_buffer({ &result.image_view }, nullptr, _pass_resources);
