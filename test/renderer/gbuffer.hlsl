...
ConstantBuffer<material_data> material : register(b1);
Texture2D<float4> base_color : register(t2);
Texture2D<float3> normal : register(t3);
Texture2D<float3> metallic_roughness : register(t4);

struct node_data {
	float4x4 transform;
};
ConstantBuffer<node_data> node : register(b0, space1);

struct global_data {
	float4x4 view;
	float4x4 projection_view;
};
ConstantBuffer<global_data> globals : register(b0, space2);
...
ps_input main_vs(vs_input input) {
	...
	output.normal = normalize(mul(node.transform, float4(input.normal.xyz, 0.0f)).xyz);
...
ps_output main_ps(ps_input input) {
	float4 base_color_val = base_color.SampleLevel(image_sampler, input.uv, 0) * material.base_color;
	if (base_color_val.a < material.alpha_cutoff) {
		discard;
	}

	float3 normal_val = normal.SampleLevel(image_sampler, input.uv, 0) * 2.0f - 1.0f;
	normal_val.xy *= material.normal_scale;
	float2 metallic_roughness_val =
		metallic_roughness.SampleLevel(image_sampler, input.uv, 0).bg *
