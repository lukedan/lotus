...
public:
	...
	void soft_reset() override {
		...
		for (u32 x = 0; x < 5; ++x) {
			for (u32 y = 0; y < 5; ++y) {
				...
				const vec3 end = start + vec3(0.0f, 0.0f, 0.2f);

				const vec3 pos_avbd(-0.5f, 0.0f, 0.0f);
				_create_straight_rod_avbd(start + pos_avbd, end + pos_avbd, _segments, _mass, _k_ss, _k_bt);

				const vec3 pos_xpbd(0.5f, 0.0f, 0.0f);
				_create_straight_rod_xpbd(start + pos_xpbd, end + pos_xpbd, _segments, _mass, _k_ss, _k_bt);
			}
		}
	}
	...
	void gui() override {
		...
		ImGui::SliderFloat("Mass", &_mass, 0.0f, 1.0f);
		ImGui::SliderFloat("Stretching-Shearing Compliance", &_k_ss, 0.0f, 10000.0f, "%.3f", ImGuiSliderFlags_Logarithmic);
		ImGui::SliderFloat("Bending Compliance", &_k_bt, 0.0f, 10000.0f, "%.3f", ImGuiSliderFlags_Logarithmic);
...
private:
	...
	template <typename Solver, typename BendCallback, typename StretchCallback> static void _create_straight_rod(
		...
		vec3 start, vec3 end, u32 num_parts, scalar total_mass
	) {
		...
		for (u32 i = 1; i < num_parts; ++i) {
			...
			ori.inv_inertia      = i < 2 ? 0.0f : 10.0f; // TODO
		}
	...
	void _create_straight_rod_xpbd(vec3 start, vec3 end, u32 num_parts, scalar total_mass, scalar k_ss, scalar k_bt) {
	...
	void _create_straight_rod_avbd(vec3 start, vec3 end, u32 num_parts, scalar total_mass, scalar k_ss, scalar k_bt) {
		_create_straight_rod(
			_solver_avbd,
